    ...
    <section className="border-b border-line/60" id="blog">
      <div className="max-w-6xl mx-auto px-6 md:px-10 py-20 md:py-28">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          ...
          <a
            href="#"
            className={`inline-flex items-center gap-2 font-mono ${monoSizes.sm} uppercase ${tracking.label} text-ink hover:text-amber link-sweep`}
          >
          ...
          {posts.map((p) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className={`font-mono ${monoSizes.xs} uppercase ${tracking.label} text-amber`}>
                  {p.tag}
                </span>
                <span className={`flex items-center gap-1 font-mono ${monoSizes.xs} text-inkSoft`}>
                  <Clock className="w-3 h-3" /> {p.readMins} min
                </span>
              </div>
              <div className={`mt-5 font-serif ${displaySizes.testimonial} ${leading.blog} text-ink group-hover:text-amber transition-colors`}>
                {p.title}
              </div>
              <p className={`mt-3 text-inkSoft ${proseSizes.md} leading-relaxed`}>
                {p.excerpt}
              </p>
              <div className="mt-auto pt-6 flex items-center justify-between">
                ...
                <span className="inline-flex items-center gap-1 text-ink group-hover:text-amber transition-colors">
                  <span className={`font-mono text-[11.5px] uppercase ${tracking.status}`}>
                    read
                  </span>
